    ...
    if file_name:
        ...
        if gene_names is not None:
            for i in range(nToWrite):
                ...
                TF_idx = int(TF_idx)
                target_idx = int(target_idx)
                results.append([(gene_names[TF_idx],gene_names[target_idx]),score])
            ...
            for i in range(nToWrite):
                ...
                TF_idx = int(TF_idx)
                target_idx = int(target_idx)
                results.append([(TF_idx,target_idx),score])
...
def GENIE3(expr_data,output_data=None,gene_names=None,regulators='all',tree_method='RF',K='sqrt',ntrees=1000,nthreads=1):
    ...
    if nthreads > 1:
        ...
        for i in range(ngenes):
            input_data.append( [expr_data,i,input_idx,tree_method,K,ntrees,output_data] )
...
def wr_GENIE3_single(args):
    return([args[1], GENIE3_single(args[0], args[1], args[2], args[3], args[4], args[5], output_data=args[6])])
    


def GENIE3_single(expr_data,output_idx,input_idx,tree_method,K,ntrees,output_data=None):
    
    ngenes = expr_data.shape[1]
    
    # Expression of target gene
    if output_data is None:
        output_data = expr_data
    output = output_data[:,output_idx]
